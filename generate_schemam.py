...
# --- ⚙️ 1. ENHANCED CONFIGURATION SYSTEM (Unchanged) ---
...
# --- 🛠️ 2. ENHANCED HELPER & SETUP FUNCTIONS (Unchanged) ---
...
# --- 🖼️ 3. ENHANCED FIGMA DATA & SUMMARIZATION (Unchanged) ---
...
# --- 🤖 4. ENHANCED AI ARCHITECT (Refactored for LangChain) ---


class PromptGenerator:
    """Dynamic prompt generation system (Unchanged)"""
...
class AIArchitect:
    """AI-powered schema architect using LangChain"""
    ...
    def __init__(self, config: AppConfig):
        ...
        self.phase_one_chain = self._create_phase_one_chain()
        self.phase_two_chain = self._create_phase_two_chain()
    ...
    def _create_phase_one_chain(self):
        """Create the LangChain for architectural planning."""
        prompt_template = PromptTemplate.from_template(
            # Using a lambda to generate the prompt dynamically on each run
        ...
        return (
            {
                "prompt_text": RunnableLambda(
                    lambda x: self.prompt_generator.generate_phase_one_prompt(
                        x["sections_summary"]
                    )
                )
            }
            ...
            | self.model
            | StrOutputParser()
            | RunnableLambda(self.text_processor.extract_json_from_response)
        )

    def _create_phase_two_chain(self):
        """Create the LangChain for schema code generation."""
        prompt_template = PromptTemplate.from_template("{prompt_text}")

        return (
            {
                "prompt_text": RunnableLambda(
                    lambda x: self.prompt_generator.generate_phase_two_prompt(
                        x["schema_name"],
                        x["classification"],
                        x["plan"],
                        x["structure_info"],
    ...
    def phase_one_architect_plan(self, sections: List[dict]) -> Optional[dict]:
        """Create architectural plan from Figma sections using a LangChain."""
        ...
        logging.debug(
            f"\n--- PHASE 1: PROMPT INPUT (SECTIONS SUMMARY) ---\n{json.dumps(sections_summary, indent=2)}\n---------------------------------"
        )

        try:
            plan = self.phase_one_chain.invoke({"sections_summary": sections_summary})
    ...
    def phase_two_generate_schema_code(
        ...
        """Generate TypeScript schema code for a specific schema using a LangChain."""
        logging.info(
        ...
        try:
            ...
            response_text = self.phase_two_chain.invoke(
                {
...
# --- 📜 5. ENHANCED SANITY FILE GENERATOR & CORRECTION (Unchanged) ---
...
# --- 📁 6. ENHANCED FILE GENERATION SYSTEM (Unchanged) ---
...
# --- 🚀 7. ENHANCED MAIN EXECUTION FLOW (Unchanged) ---
...
class SchemaArchitect:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.ai_architect = AIArchitect(config)  # Now uses the LangChain version
    ...
    def run(self):
        logging.info("🚀 AI Schema Architect (LangChain Version) Initializing... 🚀")
        try:
