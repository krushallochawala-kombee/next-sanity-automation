...
# --- Settings ---
SCHEMAS_DIR = "./src/sanity/schemaTypes"
...
def update_sanity_config_with_i18n():
    """
    Updates sanity.config.ts with internationalization configuration and correct schema import.
    """
    ...
    try:
        ...
        logging.info(
            "🔧 Updating sanity.config.ts with internationalization configuration..."
        )
        ...
        if (
            ...
            logging.info(
                "✅ sanity.config.ts already has internationalization configured"
            )
            return True

        # Updated configuration template matching the user's desired format
        new_config_content = """'use client'
...
"""
        ...
        logging.info("   - SUPPORTED_LOCALES (English & Hindi)")
        logging.info("   - Correct schema import path (../schemaTypes)")
        logging.info("   - Field types: string, text, image, url, file, slug")
